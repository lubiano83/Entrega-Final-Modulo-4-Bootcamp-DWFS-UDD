...
export default class LodgesController {
    ...
    createLodge = async(req, res) => {
        try {
            ...
            const modifiedData = { hotel: hotel.toLowerCase().trim(), img: [], size: Number(size), bedroom: Number(bedroom), bathroom: Number(bathroom), capacity: Number(capacity), wifi: false, season: { high: Number(high), medium: Number(medium), low: Number(low) }, available: false };
    ...
    updateLodgeById = async(req, res) => {
        try {
            ...
            const modifiedWifi = String(wifi).toLowerCase().trim() === "true" ? true : String(wifi).toLowerCase().trim() === "false" ? false : wifi;
            if(typeof modifiedWifi !== "boolean") return res.status(400).send({ message: "El campo wifi debe ser true o false.." });
            const modifyData = { hotel: hotel, size: Number(size), bedroom: Number(bedroom), bathroom: Number(bathroom), capacity: Number(capacity), wifi: modifiedWifi, season: { high: Number(season.high), medium: Number(season.medium), low: Number(season.low) }};
